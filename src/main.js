...
function drawChar(c,sx,sy,dir,frame,pal){
...
function drawSit(c,sx,sy,o){ sx|=0; sy|=0;
...
const PAL_PLAYER={coat:'#caa23a',coatHi:'#e0bb55',coatLo:'#a07f28',pants:'#3a4a66',shoe:'#2a2118',skin:C.skinA,skinHi:'#f0d0a8',hair:'#5a3a22'};
const PAL_OMA={coat:'#8a6a8a',coatHi:'#a384a3',coatLo:'#6e526e',pants:'#4a4452',shoe:'#2a2118',skin:'#e8c2a0',hair:'#cfc7c2'};
const PAL_KID={coat:'#3a8a6a',coatHi:'#4fa080',coatLo:'#2c6e52',pants:'#7a4a2a',shoe:'#2a2118',skin:'#e8c39a',hair:'#3a2a1a'};
// Haze — blond, langer Hals, chilliger Hoodie
const PAL_HAZE={coat:'#5d7f8e',coatHi:'#7a9aa8',coatLo:'#46606c',pants:'#3a3f4a',shoe:'#2a2118',skin:'#e8c39a',skinHi:'#f0d0a8',hair:'#e6c862',neck:true};
// Passi — blonder Lockenkopf, Berkeley-College-Vibe
const PAL_PASSI={coat:'#3a5a8a',coatHi:'#5074aa',coatLo:'#2c4468',pants:'#5a4a36',shoe:'#2a2118',skin:'#e8c39a',skinHi:'#f0d0a8',hair:'#e8cc6a',curly:true};

const player={x:16*TILE, y:11*TILE, dir:'down', frame:0, step:0, speed:54};
const npcs=[
...
function facingTo(a,b){ const dx=b.x-a.x, dy=b.y-a.y; if(Math.abs(dx)>Math.abs(dy)) return dx>0?'right':'left'; return dy>0?'down':'up'; }
...
function movePlayer(dt,blockFn){
  let dx=0,dy=0;
...
// ---------- Sprite: Geeked up Racoon (Battle, frontal) ----------
const RAC={ out:'#221f1c', dk:'#34302a', gy:'#5f574d', gyH:'#776d61', gyL:'#4a443c',
...
function drawRacoonBattle(c, ox, oy, t){
...
const LEOP={ out:'#3a2c20', fur:'#6a5240', furH:'#7e6450', furL:'#4e3c2e', str:'#3f3022', ear:'#b89a8a' };
function drawLeoBack(c, ox, oy){
...
function drawGigoStub(c, ox, oy, t){
...
const MOVES = {
  ...
  fauchen:{ name:'Fauchen', pow:0, type:'Normal', acc:1.0, eff:'atkdown', desc:'Lautes Fauchen senkt den Angriff des Gegners.' },
...
function drawEvoMon(id, cx, bottomY, scale, white, alpha){ const g=GIGODEX[id]; if(!g||!g.draw) return;
  ...
  if(white) GIGO_WHITE=true; try{ g.draw(X,-24,-46,evolveState.t); }catch(_){}; GIGO_WHITE=false; X.restore(); X.globalAlpha=1; }
...
function bdMon(id,cx,cy,s,grey){ const g=GIGODEX[id]; if(!g||!g.draw){ X.fillStyle='#b7ab8e'; X.font='bold 15px Georgia'; X.textAlign='center'; X.textBaseline='middle'; X.fillText('?',cx,cy); X.textAlign='left'; X.textBaseline='top'; return; }
  if(typeof GIGO_IMG!=='undefined' && GIGO_IMG[id]){ if(grey)GIGO_GREY=true; X.save(); X.translate(cx,cy); X.scale(s,s); X.translate(-24,-27); g.draw(X,0,0,T); X.restore(); GIGO_GREY=false; return; }
...
// ---------- Starter-Sprites (Battle-Scale, Platzhalter-Qualitaet) ----------
function drawMephe(c,ox,oy,t){ t=t||0; const x=v=>ox+v,y=v=>oy+v; const P={ O:'#2a1c12', fur:'#6f4a2c', furH:'#875c38', furD:'#4a3320', sk:'#e2b488', skH:'#f0caa0', skD:'#c99a6e',
...
function drawEcstasy(c,ox,oy,t){ t=t||0; const x=v=>ox+v,y=v=>oy+v; const P={ O:'#5a3244', f:'#d89ab0', fH:'#e8b4c6', fD:'#b87a94', fDD:'#9a6280',
...
function drawKoks(c,ox,oy,t){ t=t||0; const x=v=>ox+v,y=v=>oy+v; const P={ O:'#233618', g:'#5f8a3f', gH:'#7aa851', gD:'#476a2c', moss:'#4f7a34', mossD:'#3a5c26',
...
const GIGO_IMG={}; const GIGO_IMG_GREY={}; let GIGO_GREY=false;
const GIGO_IMG_WHITE={}; let GIGO_WHITE=false;
const GIGO_IMG_SRC={
...
function greyOf(k){ const im=GIGO_IMG[k]; if(!im||!im.complete||!im.width) return null; const cached=GIGO_IMG_GREY[k]; if(cached&&cached._w===im.width) return cached;
...
function whiteOf(k){ const im=GIGO_IMG[k]; if(!im||!im.complete||!im.width) return null; const cached=GIGO_IMG_WHITE[k]; if(cached&&cached._w===im.width) return cached;
...
function drawGigoImg(k,c,ox,oy){ const im=GIGO_IMG[k]; if(!im||!im.width) return; const g=GIGODEX[k]; const ds=(g&&g.dispScale)||1; const dy=(g&&g.dispDy)||0;
  const src = (GIGO_WHITE && whiteOf(k)) || (GIGO_GREY && greyOf(k)) || im; const hh=48*ds, ww=im.width*(hh/im.height);
...
function drawKraehe1(c,ox,oy,t){ drawGigoImg('kraehe',c,ox,oy); }
function drawKraehe2(c,ox,oy,t){ drawGigoImg('kraehe2',c,ox,oy); }
function drawKraehe3(c,ox,oy,t){ drawGigoImg('kraehe3',c,ox,oy); }
function drawSquirrel1(c,ox,oy,t){ drawGigoImg('squirrel',c,ox,oy); }
function drawSquirrel2(c,ox,oy,t){ drawGigoImg('squirrel2',c,ox,oy); }
function drawSquirrel3(c,ox,oy,t){ drawGigoImg('squirrel3',c,ox,oy); }
function drawKrabbe1(c,ox,oy,t){ drawGigoImg('krabbe',c,ox,oy); }
function drawKrabbe2(c,ox,oy,t){ drawGigoImg('krabbe2',c,ox,oy); }
